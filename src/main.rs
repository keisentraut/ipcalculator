...
impl IpRange for Ipv4Range {
    ...
    fn _reduce_cidr_by_one(&mut self) -> &mut Self {
        match self.cidr {
            0 => self,
            n => {self.cidr = n-1; self.normalize() }
        }
    }
    fn is_subset_of(&self, other: &Self) -> bool {
        self.cidr >= other.cidr && 
            self.clone()._set_cidr(other.cidr).normalize().ip == other.ip
    }
    fn is_superset_of(&self, other: &Self) -> bool {
        self.cidr <= other.cidr && 
            other.clone()._set_cidr(self.cidr).normalize().ip == self.ip
    }
    fn merge_with(&self, other: &Self) -> Option<Self> {
...
impl FromStr for Ipv4Range {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        match (slashes.len(), slashes[0].parse::<Ipv4Addr>()) {
            ...
            (1,  Ok(i)) => Ok(Ipv4Range { ip: i.into(), cidr: 32 }),
            (2,  Ok(i)) => match slashes[1].parse() {
                Ok(n) => match n {
                   0...32 => { let mut res = Ipv4Range { ip: i.into(), cidr: n }; res.normalize(); Ok(res) },
...
impl FromStr for Ipv6Range {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        match (slashes.len(), slashes[0].parse::<Ipv6Addr>()) {
            ...
            (1,  Ok(i)) => Ok(Ipv6Range { ip: i.into(), cidr: 32 }),
            (2,  Ok(i)) => match slashes[1].parse() {
                Ok(n) => match n {
                   0...128 => { let mut res = Ipv6Range { ip: i.into(), cidr: n }; res.normalize(); Ok(res) },
                   _ => Err(RangeParseError::CidrInvalid),
                },
                Err(_) => Err(RangeParseError::CidrInvalid),
            },
...
mod tests {
    ...
    fn sub_and_superset2a() {
        let i : Ipv6Range = "ffff:ffff:ffff:ffff:ffff:ffff:ffff:ffff/128".parse().unwrap();
        for c in 0..127 {
    ...
    fn add_v4() {
        ...
        rand::thread_rng().shuffle(&mut tmp);
        for i in 0..256 {
            l.add_v4(tmp[i]);
            println!("***************************************");
            println!("{}", tmp[i]);
        ...
        assert!(l.v4.len() == 1);
        assert!(l.v4[0] == Ipv4Range { ip: 192*256*256*256+168*256*256, cidr: 16} );
    }
}
